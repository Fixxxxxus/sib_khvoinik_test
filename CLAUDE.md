...
- Сетка скидок за объём - единственное место `pages/wholesale_pricing.py`: `DISCOUNT_TIERS` (пороги и проценты), `DISCOUNT_BASIS` (`amount` или `quantity`), `DISCOUNT_TIERS_APPROVED`, `MIN_ORDER_AMOUNT` (минимальная сумма заказа, заглушка до утверждения). Ценовая лестница карточки («Без скидки» плюс ступени с порогами) считается там же функцией `price_ladder()` из розничной цены: отдельных полей под ступени в БД нет. Полоса прогресса и подсказка «сколько добрать» - `progress_hint()` на сервере и тот же расчёт в `static/js/opt.js`. Тот же конфиг уезжает в JS корзины через `<script id="opt-discount-config">`.
