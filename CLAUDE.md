...
- Сетка скидок за объём - единственное место `pages/wholesale_pricing.py`. Сетка ГИБРИДНАЯ: часть ступеней меряется штуками, часть чеком; ступень считается взятой, если выполнена ЛЮБАЯ её ось (`min_quantity` ИЛИ `min_amount`), применяется самая выгодная для клиента. Подтверждена заказчиком 10.09.2026: 20% с первой штуки (оптовая цена по умолчанию = розница минус 20%), 25% от 30 шт, 30% от 50 шт, 35% при чеке от 100 000 ₽, от 100 шт или чека 200 000 ₽ - индивидуальное предложение (процента нет, показываем текст `INDIVIDUAL_TIER_TEXT`, цену подтверждает менеджер; флаг `INDIVIDUAL_TIER_NEEDS_MANUAL_APPROVAL`). Минимальный заказ `MIN_ORDER_AMOUNT = 15 000 ₽`. Ценовая лестница карточки («Розница» плюс ступени) считается там же в `price_ladder()` из розничной цены: отдельных полей под ступени в БД нет. Подсказка «сколько добрать» (`next_tier_for`, `progress_hint`) выбирает ближайшую ступень по обеим осям сразу: сравнивает остаток в долях порога, при примерно равной близости (`HINT_TOLERANCE`) ведёт к нижней ступени лестницы. Тот же расчёт продублирован в `static/js/opt.js` и получает конфиг через `<script id="opt-discount-config">`.
