...
- Модели: `WholesaleSection`, `WholesaleItem`, `WholesaleItemVariant`, `WholesaleOrder`, `WholesaleOrderLine` (`pages/models.py`). Вариант позиции (цвет, ком, партия) несёт свой остаток, необязательную свою цену и фото; корзина различает варианты одной позиции, количество сверху режется остатком. Наполнение - Django admin, разделы «оптовый каталог: …». Заказы только на чтение.
